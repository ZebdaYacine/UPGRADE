 ...
 * @author Zed-Yacine
 */
public class AddSpecialitiesUIController implements Initializable {
    ...
    private void addClient(ActionEvent event) throws IOException, SQLException {
        Speciality sp = new Speciality(name.getText());
        boolean statusData = sp.isDataEmpty(sp);
        if (statusData) {
            Options.information(SpecialityController.addSpeciality(sp) + "");
            refrechData();
        } else {
