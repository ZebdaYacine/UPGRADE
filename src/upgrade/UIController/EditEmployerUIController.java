 ...
 * @author Zed-Yacine
 */
public class EditEmployerUIController implements Initializable {
    ...
    @FXML
    private JFXTextField id, Fname, Lname, phone, nbrChildren, deploma, formations, experience, note;
    ...
    private void deleteUI(ActionEvent event) throws IOException, SQLException {
        Employer empl = new Employer(getInt(id.getText()));
        if (empl.dateIsValid()) {
            Options.information(EmployerController.deleteEmployer(empl) + "");
            refrechData();
        }
    }
